    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-1 border-r pr-4">
            <div class="flex justify-between items-center mb-4">
                ...
                <button onclick="openSectionModal()" class="text-blue-600 hover:text-blue-800 text-sm flex items-center gap-1">
            ...
            <div id="sections-list" class="space-y-2">
                ...
                <div class="section-item group">
                    <button onclick="loadSectionContent({{ $section->id }})" 
                        ...
                        <div class="opacity-0 group-hover:opacity-100 transition">
                            <button onclick="editSection({{ $section->id }}, '{{ $section->name }}')" class="text-gray-400 hover:text-blue-600 mr-1">
                ...
                <div class="text-center py-8">
                    ...
                    @if(auth()->user()->isSuperAdmin() || auth()->user()->canAccess('intercession', 'manage-archives'))
                    <button onclick="openSectionModal()" class="mt-3 text-blue-600 hover:text-blue-800 text-sm">
        ...
        <div class="md:col-span-2">
            <div class="flex justify-between items-center mb-4">
                ...
                @if(auth()->user()->isSuperAdmin() || auth()->user()->canAccess('intercession', 'manage-archives'))
                <button onclick="openPageModal()" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded-lg text-sm flex items-center gap-1">
    ...
    <div class="relative top-40 mx-auto p-5 border w-full max-w-md shadow-lg rounded-lg bg-white">
        ...
        <form id="section-form" method="POST">
            ...
            <div class="flex justify-end gap-2 mt-5 pt-3 border-t">
                ...
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">Save Section</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-lg bg-white">
        ...
        <form id="page-form" method="POST">
            ...
            <div class="mt-4 space-y-3">
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" id="page_is_published" name="is_published" value="1" class="w-4 h-4 text-blue-600 rounded">
            ...
            <div class="flex justify-end gap-2 mt-5 pt-3 border-t">
                ...
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">Save Page</button>
            </div>
        </form>
    </div>
</div>
...
function deleteSection(id) {
    if (confirm('Delete this section? All pages in this section will also be deleted.')) {
        fetch(`/intercession/archives/sections/${id}`, {
            method: 'DELETE',
            headers: {
...
function loadSectionContent(sectionId) {
    ...
    fetch(`/intercession/archives/sections/${sectionId}/pages`, {
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            ...
            if (data.pages.length > 0) {
                    ...
                    <div class="border rounded-lg p-4 hover:shadow-md transition cursor-pointer" onclick="loadPageContent(${page.id})">
                        <div class="flex justify-between items-start">
                            ...
                            <div class="flex gap-2 ml-4">
                                <button onclick="event.stopPropagation(); editPage(${page.id})" class="text-gray-400 hover:text-blue-600">
                ...
                pagesContainer.innerHTML = `
                    <div class="text-center py-12">
                        ...
                        <button onclick="openPageModal()" class="mt-3 text-blue-600 hover:text-blue-800 text-sm">
...
function editPage(pageId) {
    fetch(`/intercession/archives/pages/${pageId}/edit`, {
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'Accept': 'application/json'
        }
...
function deletePage(pageId) {
    if (confirm('Delete this page?')) {
        fetch(`/intercession/archives/pages/${pageId}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                'X-Requested-With': 'XMLHttpRequest'
...
function loadPageContent(pageId) {
    window.open(`/intercession/archives/pages/${pageId}`, '_blank');
}
...
document.getElementById('section-form')?.addEventListener('submit', function(e) {
    ...
    if (method === 'PUT' && sectionId) {
        url = `/intercession/archives/sections/${sectionId}`;
