...
<div class="min-h-screen bg-[#f0ebf8] py-10">
    ...
    <div class="max-w-5xl mx-auto mb-4 flex justify-between items-center">
        <a href="{{ route('forms.manage.index') }}" class="inline-flex items-center text-indigo-600 hover:text-indigo-800 text-sm">
            <i class="fas fa-arrow-left mr-2"></i> Back to Manage Forms
        </a>
        <div class="flex gap-3">
            <button onclick="saveAndReturn()" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm">
        ...
        <div class="bg-white rounded-2xl border border-gray-300 overflow-hidden">
            ...
            <div class="p-6">
                ...
                <div id="quiz-settings-content" class="settings-content">
                    ...
                    <div id="quizDetails" class="{{ ($form->settings['is_quiz'] ?? false) ? '' : 'hidden' }}">
                        <div class="py-4 border-b"><h3 class="font-medium text-gray-800 mb-2">Release grade</h3>
                            <label class="flex items-center gap-2"><input type="radio" name="release_grade" value="immediately" {{ ($form->settings['release_grade'] ?? 'immediately') == 'immediately' ? 'checked' : '' }} class="text-indigo-600"><span class="text-sm">Immediately after submission</span></label>
                            <label class="flex items-center gap-2"><input type="radio" name="release_grade" value="later" {{ ($form->settings['release_grade'] ?? '') == 'later' ? 'checked' : '' }} class="text-indigo-600"><span class="text-sm">Later, after manual review</span></label>
                        </div>
                        <div class="py-4"><h3 class="font-medium text-gray-800 mb-2">Default points</h3>
                            <input type="number" id="defaultPoints" value="{{ $form->settings['default_points'] ?? 1 }}" min="1" class="w-20 px-2 py-1 border rounded-md text-center"><span class="text-sm ml-2">points per question</span>
                        </div>
                    </div>
                </div>
                ...
                <div id="responses-settings-content" class="settings-content hidden">
                    <div class="flex justify-between items-start py-4 border-b">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" id="allowViewResponse" class="sr-only peer" {{ ($form->settings['allow_view_response'] ?? true) ? 'checked' : '' }}>
                            <div class="w-11 h-6 bg-gray-300 rounded-full peer peer-checked:bg-indigo-600"></div>
                            <div class="absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-all peer-checked:translate-x-5"></div>
                        </label>
                    </div>
                ...
                <div id="presentation-settings-content" class="settings-content hidden">
                    <div class="py-4 border-b"><label class="flex items-center gap-2"><input type="checkbox" id="showProgressBar" class="rounded text-indigo-600" {{ ($form->settings['show_progress_bar'] ?? false) ? 'checked' : '' }}><span class="text-sm">Show progress bar</span></label></div>
                    <div class="py-4 border-b"><label class="flex items-center gap-2"><input type="checkbox" id="shuffleQuestions" class="rounded text-indigo-600" {{ ($form->settings['shuffle_questions'] ?? false) ? 'checked' : '' }}><span class="text-sm">Shuffle question order</span></label></div>
                    <div class="py-4"><label class="block text-sm text-gray-700 mb-1">Confirmation message</label>
                        <textarea id="confirmationMessage" rows="2" class="w-full border rounded-lg px-3 py-2 text-sm">{{ $form->settings['confirmation_message'] ?? 'Your response has been recorded.' }}</textarea>
                    </div>
                </div>

                <!-- Defaults Settings -->
                <div id="defaults-settings-content" class="settings-content hidden">
                    <div class="py-4 border-b"><label class="flex items-center gap-2"><input type="checkbox" id="defaultRequired" class="rounded text-indigo-600" {{ ($form->settings['default_required'] ?? false) ? 'checked' : '' }}><span class="text-sm">Make questions required by default</span></label></div>
                    <div class="py-4"><label class="flex items-center gap-2"><input type="checkbox" id="publishByDefault" class="rounded text-indigo-600" {{ ($form->settings['publish_by_default'] ?? false) ? 'checked' : '' }}><span class="text-sm">Publish form by default</span></label></div>
                </div>
            </div>
...
let questionCount = 0;
let questions = [];
let isQuizMode = {{ ($form->settings['is_quiz'] ?? false) ? 'true' : 'false' }};
...
function saveSettings() {
    ...
    fetch('/forms/manage/{{ $form->id }}/settings', {
        method: 'POST',
        headers: {
...
function loadSettings() {
    if (document.getElementById('allowViewResponse')) document.getElementById('allowViewResponse').checked = {{ ($form->settings['allow_view_response'] ?? true) ? 'true' : 'false' }};
    if (document.getElementById('allowEditing')) document.getElementById('allowEditing').checked = {{ ($form->settings['allow_editing'] ?? false) ? 'true' : 'false' }};
    if (document.getElementById('limitOneResponse')) document.getElementById('limitOneResponse').checked = {{ ($form->settings['limit_one_response'] ?? false) ? 'true' : 'false' }};
    if (document.getElementById('shuffleQuestions')) document.getElementById('shuffleQuestions').checked = {{ ($form->settings['shuffle_questions'] ?? false) ? 'true' : 'false' }};
    if (document.getElementById('showProgressBar')) document.getElementById('showProgressBar').checked = {{ ($form->settings['show_progress_bar'] ?? false) ? 'true' : 'false' }};
    if (document.getElementById('defaultRequired')) document.getElementById('defaultRequired').checked = {{ ($form->settings['default_required'] ?? false) ? 'true' : 'false' }};
    if (document.getElementById('publishByDefault')) document.getElementById('publishByDefault').checked = {{ ($form->settings['publish_by_default'] ?? false) ? 'true' : 'false' }};
...
function loadQuestions() {
    ...
    let questionsData = existingQuestions;
    ...
    if (questionsData && Array.isArray(questionsData) && questionsData.length > 0) {
        questionsData.forEach((q, index) => {
            let question = {
                ...
                options: q.options || (q.type === 'multiple_choice' || q.type === 'checkboxes' || q.type === 'dropdown' ? ['Option 1'] : null),
                rows: q.rows || (q.type === 'multiple_choice_grid' || q.type === 'checkbox_grid' ? ['Row 1'] : null),
                columns: q.columns || (q.type === 'multiple_choice_grid' || q.type === 'checkbox_grid' ? ['Column 1'] : null),
...
function saveForm(isAutoSave = false) {
    ...
    fetch('{{ route("forms.manage.update", $form->id) }}', {
        method: 'PUT',
    ...
    .then(data => {
        if (data.success) {
            if (isAutoSave) {
                ...
                window.location.href = '/intercession';
            }
        } else {
            if (!isAutoSave) alert('Error: ' + data.message);
        }
    })
    .catch(error => { console.error('Error:', error); if (!isAutoSave) alert('Error updating form'); });
}
