<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
        <div class="rounded-xl border border-sky-100 bg-gradient-to-br from-white via-sky-50 to-blue-50/40 p-3 sm:p-4 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xl sm:text-2xl md:text-3xl font-bold text-slate-900 tracking-tight" id="total_requests">0</p>
        ...
        <div class="rounded-xl border border-amber-100 bg-gradient-to-br from-white via-amber-50 to-yellow-50/50 p-3 sm:p-4 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xl sm:text-2xl font-bold text-amber-600" id="pending_requests">0</p>
        ...
        <div class="rounded-xl border border-emerald-100 bg-gradient-to-br from-white via-emerald-50 to-teal-50/40 p-3 sm:p-4 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xl sm:text-2xl font-bold text-emerald-600" id="approved_requests">0</p>
        ...
        <div class="rounded-xl border border-rose-100 bg-gradient-to-br from-white via-rose-50 to-red-50/40 p-3 sm:p-4 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xl sm:text-2xl font-bold text-rose-600" id="rejected_requests">0</p>
    ...
    @if($canView)
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 items-end rounded-2xl border border-gray-100 bg-white/90 p-4 shadow-sm backdrop-blur">
        ...
        <button onclick="filterPermissions()" class="w-full rounded-xl bg-gray-100 px-4 py-2.5 text-sm font-medium text-gray-700 transition hover:bg-gray-200">
            <i class="fas fa-search mr-1"></i> Filter
        </button>
        <button onclick="exportPermissions()" class="w-full rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-medium text-white shadow-sm transition hover:bg-blue-700">
...
function renderPermissionsList(permissions) {
    ...
    if (!permissions || permissions.length === 0) {
        container.innerHTML = `
            <div class="rounded-2xl border border-gray-100 bg-white p-8 text-center text-gray-500 shadow-sm">
                ...
                @if($canCreate)
                <button onclick="openPermissionModal()" class="mt-3 text-sm font-medium text-blue-600 hover:text-blue-700">
    ...
    const normalized = permissions.map(perm => {
        ...
        const commentLines = perm.status === 'approved'
            ? [`Approver: ${approverName}`, 'Comment: Approved']
            : perm.status === 'rejected'
                ? [`Approver: ${approverName}`, `Comment: ${rejectionReason}`]
                : ['Pending'];

        let statusConfig = { class: 'bg-amber-100 text-amber-700 ring-1 ring-amber-200', icon: 'fa-clock', text: 'Pending' };
        ...
        return { ...perm, totalDays, commentLines, statusConfig };
    });
            ...
            ${normalized.map(perm => `
                <div class="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            <p class="font-semibold text-gray-900 truncate">${escapeHtml(perm.user_name || 'N/A')}</p>
                            <p class="text-sm text-gray-500">${formatDate(perm.start_date)} - ${formatDate(perm.end_date)}</p>
                        </div>
                        <span class="shrink-0 inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold ${perm.statusConfig.class}">
                            <i class="fas ${perm.statusConfig.icon}"></i>${perm.statusConfig.text}
                        </span>
                    </div>
                    <p class="mt-3 text-sm text-gray-700 break-words">${escapeHtml(perm.reason || '-')}</p>
                    <div class="mt-3 flex flex-wrap gap-2 text-xs text-gray-500">
                        <span class="rounded-full bg-gray-100 px-2 py-1">${perm.totalDays} day${perm.totalDays === 1 ? '' : 's'}</span>
                        ${perm.commentLines.map(line => `<span class="rounded-full bg-gray-50 px-2 py-1">${escapeHtml(line)}</span>`).join('')}
                    </div>
                    ${perm.status === 'pending' ? `
                        <div class="mt-4 grid grid-cols-2 gap-2">
                            @if($canApprove)
                            <button onclick="approvePermission(${perm.id})" class="rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-sm font-medium text-green-700">Approve</button>
                            @endif
                            @if($canReject)
                            <button onclick="rejectPermission(${perm.id})" class="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm font-medium text-red-700">Reject</button>
                            @endif
                        </div>
                    ` : ''}
                </div>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        ${normalized.map(perm => `
                            <tr class="hover:bg-gray-50/80">
                                <td class="px-4 py-4 align-top"><div class="font-medium text-gray-900">${escapeHtml(perm.user_name || 'N/A')}</div></td>
                                <td class="px-4 py-4 align-top"><div class="max-w-xl whitespace-normal break-words text-gray-700">${escapeHtml(perm.reason || '-')}</div></td>
                                ...
                                <td class="px-4 py-4 align-top text-center font-semibold text-gray-900">${perm.totalDays}</td>
                                <td class="px-4 py-4 align-top">
                                    <div class="flex flex-col gap-2">
                                        <span class="inline-flex w-fit items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold ${perm.statusConfig.class}">
                                            <i class="fas ${perm.statusConfig.icon}"></i>${perm.statusConfig.text}
                                        </span>
                                        <div class="flex flex-wrap items-center gap-1">
                                            ${perm.status === 'pending' ? `
                                                @if($canApprove)
                                                <button onclick="approvePermission(${perm.id})" class="rounded-lg border border-green-200 bg-green-50 px-2 py-1 text-xs font-medium text-green-700 transition hover:bg-green-100">Approve</button>
                                                @endif
                                                @if($canReject)
                                                <button onclick="rejectPermission(${perm.id})" class="rounded-lg border border-red-200 bg-red-50 px-2 py-1 text-xs font-medium text-red-700 transition hover:bg-red-100">Reject</button>
                                                @endif
                                            ` : ''}
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-4 align-top"><div class="max-w-xl space-y-1 whitespace-normal break-words text-gray-700">${perm.commentLines.map(line => `<div>${escapeHtml(line)}</div>`).join('')}</div></td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
